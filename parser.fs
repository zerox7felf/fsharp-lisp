...
module Parser = 
    ...
    let rec parser
        ...
        let rec parseNumber (input: string) ((index, line, col): ParserPosition)  numberBuf symbolTable parsedSubexpressions =
            if input = "" then
                parser input (index, line, col)  symbolTable (
        ...
        // Parse comment
        let rec parseComment input ((index, line, col): ParserPosition) symbolTable parsedSubexpressions = 
            if input = "" then
                parser input (index, line,col) symbolTable parsedSubexpressions ""
            else
                match input.[0] with
                | '\n' ->
                    parser input (index,line,col) symbolTable parsedSubexpressions ""
                | _ ->
                    parseComment input.[1..] (index+1,line,col+1) symbolTable parsedSubexpressions
        ...
        if input = "" then
            ...
            | _ when Char.IsWhiteSpace(Convert.ToChar(input.[0])) ->
                ...
                if currWord = "" then
                    ...
                    //Comment
                    parseComment input (index, line, col) (SymbolTable symbolTable) parsedSubexpressions
                else
